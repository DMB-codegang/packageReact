...
function PackageListPage() {
  ...
  const handleFileUpload = async (file: File) => {
    if (!file.type.startsWith('image/')) {
      message.error('请上传图片文件（JPG、PNG等格式）');
      return false;
    }
    
    // 检查文件大小（限制为10MB）
    if (file.size > 10 * 1024 * 1024) {
      message.error('图片文件过大，请上传小于5MB的图片');
      return false;
    }
      ...
      try {
        ...
        reader.onloadend = async () => {
          try {
            ...
            // 执行OCR识别
            const ocrResults = await ocr(base64Image);
            ...
            setOcrProcessing(false);
            message.success(`成功识别到 ${ocrResults.length} 个文本`);
            ...
            message.error(`OCR识别失败: ${error instanceof Error ? error.message : '请重试'}`);
            setOcrProcessing(false);
            resolve(false);
          }
        };
  ...
  const handleUseOcrResult = async () => {
    ...
    try {
      ...
      const extractedInfo = extractPackageInfo(ocrResults);
      if (!extractedInfo) {
        message.warning('未从OCR结果中提取到有效快递信息');
        setFormLoading(false);
        return false;
      }
